...
export async function fetchTranscriptAction(videoId: string, lang: string = 'en'): Promise<{ fullText: string; segments: { text: string; offset: number }[]; error?: string }> {
  ...
  try {
    ...
    // METHOD 1: NoteGPT API (fastest — simple HTTP GET, ~2s, works from any IP)
    let transcript = await fetchFromNoteGPT(videoId, lang);
    
    if (!transcript || transcript.length === 0) {
      console.log('[Method 1 NoteGPT] Failed, trying youtube-transcript lib...');
      
      // METHOD 2: youtube-transcript library (fast, ~2s)
      try {
        ...
        console.log('[Method 2 youtube-transcript] Failed:', e.message);
      }
    }
    
    if (!transcript || transcript.length === 0) {
      console.log('[Method 2] Failed, trying Innertube...');
      
      // METHOD 3: Innertube API (heavier, 3-8s, can fail on Vercel IPs)
      transcript = await getAutoGeneratedCaptions(videoId, lang, clientIp);
    }
    
    if (!transcript || transcript.length === 0) {
      console.log('[Method 3] Failed, trying watch page HTML...');
      
      // METHOD 4: Extract from watch page HTML
      transcript = await extractFromWatchPage(videoId, clientIp);
    }

    if (!transcript || transcript.length === 0) {
      console.log('[Method 4] Failed, using metadata fallback...');
